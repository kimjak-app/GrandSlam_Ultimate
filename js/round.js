...
function setRoundOpt(opt) {
  ...
  if (opt === 'manual' && roundMode === 'double') updateRoundManualTeamPreview();
...
function setRoundMode(mode) {
  ...
  if (roundOpt === 'manual' && mode === 'double') updateRoundManualTeamPreview();
  else $('round-manual-team-box').style.display = 'none';
  checkRoundGenButton();
}
...
function updateRoundCount() {
  ...
  if (roundOpt === 'manual' && roundMode === 'double') updateRoundManualTeamPreview();
}
...
function saveRoundResults() {
  ...
  gsConfirm(`${finishedMatches.length}경기의 결과를 저장하시겠습니까?`, ok => {
    ...
    finishedMatches.forEach(m => {
      ...
      _applyRoundScore(winner, loser, roundMode, winPoint, losePoint);
    });
    ...
    sorted.forEach((s, idx) => {
      ...
      _applyRoundBonus(s.name, roundMode, bonus);
    });
    ...
    roundParticipants.forEach(participant => {
      ...
      if (standings[key] && standings[key].matches === 0) {
        _applyRoundBonus(participant, roundMode, TENNIS_RULES.roundBonus[5]);
      }
    });
...
function _applyRoundScore(winner, loser, mode, winPoint, losePoint) {
...
function _applyRoundBonus(participant, mode, bonus) {
  if (mode === 'single') {
...
async function convertRoundToTournament() {
  const finishedMatches = roundMatches.filter(m => m.winner !== null);
  if (finishedMatches.length === 0) { gsAlert('완료된 경기가 없습니다.'); return; }
  if (isPracticeMode !== 'practice') await saveRoundDataToLog(finishedMatches);

  const standings = _calcStandings(finishedMatches);
  const sorted = _sortStandings(standings);
  openTournamentModal(sorted.map(s => s.name));
}
...
async function saveRoundDataToLog(finishedMatches) {
  ...
  finishedMatches.forEach(m => {
    ...
    _applyRoundScore(winner, loser, roundMode);
  });
...
// 공용 standings 계산 헬퍼
function _calcStandings(finishedMatches) {
  const standings = {};
  roundParticipants.forEach(p => {
    const key = roundMode === 'single' ? p : p.join('&');
...
function _sortStandings(standings) {
...
function startTournamentFromModal() {
  ...
  const standings = _calcStandings(roundMatches.filter(m => m.winner !== null));
  const sorted = _sortStandings(standings);
  ...
  for (let i = 0; i < bracketSize / 2; i++) {
    ...
    if (high !== null && low !== null) {
      ...
      if (isPracticeMode !== 'practice') {
        _applyRoundBonus(high, roundMode, 1);
        computeAll();
      }
    }
  }
...
function setMiniTournamentWinner(matchId, side) {
  ...
  if (isPracticeMode !== 'practice') {
    _applyRoundBonus(winner, roundMode, 1);
  }
